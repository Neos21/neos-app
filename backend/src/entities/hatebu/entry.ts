 ...
 * - スクレイピング時、当該カテゴリの既存レコードを全て削除してから登録するので、レコードの更新は発生しない (必ず削除 → 登録)
 */
@Entity('hatebu_entries')
export class Entry {
  /** ID */
  ...
  @ManyToOne(() => require('./category').Category, (category: Category) => category.entries, { createForeignKeyConstraints: false })
